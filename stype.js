...
ig.module('game.menus.detailed-stats').requires('game.menus.base', 'game.xhr').defines(function() {
    DetailedStats = ig.Class.extend({
        ...
        scoreScale: 1,
        init: function() {
            this.container = ig.$('#stype-stats');
            this.closeButton = ig.$('#stype-stats-close');
            this.content = ig.$('#stype-stats-content');
            this.canvas = ig.$('#stype-stats-canvas');
            this.context = this.canvas.getContext('2d');
        },
        show: function() {
            ...
            ig.$('#stype-stats-share-facebook').addEventListener('click', this.shareFacebookBound);
            this.shareTwitterBound = this.shareTwitter.bind(this);
            ig.$('#stype-stats-share-twitter').addEventListener('click', this.shareTwitterBound);
        },
        ...
        close: function() {
            ...
            ig.$('#stype-stats-share-facebook').removeEventListener('click', this.shareFacebookBound);
            ig.$('#stype-stats-share-twitter').removeEventListener('click', this.shareTwitterBound);
        ...
        shareFacebook: function() {
            ...
            this.saveScoresScreenshot(function(response) {
                ...
                shareWindow.location.href = 'https://www.facebook.com/dialog/feed' + '?app_id=112268002182064' + '&display=popup' + '&caption=' + encodeURIComponent('My stype Scores') + '&link=' + encodeURIComponent('http://zty.pe/') + '&picture=' + encodeURIComponent(imageUrl);
            });
        },
        shareTwitter: function() {
            ...
            this.saveScoresScreenshot(function(response) {
                var imageUrl = 'http://zty.pe/' + response.file;
                shareWindow.location.href = 'http://twitter.com/share' + '?text=' + encodeURIComponent('My #stype Scores: ' + imageUrl + ' - ')
            });
        }
    });
});
...
ig.module('game.menus.title').requires('game.menus.base', 'game.menus.detailed-stats', 'game.ease').defines(function() {
    ...
    MenuItemAboutTrainer = MenuItem.extend({
        ...
        ok: function() {
            window.location.href='https://github.com/KevinWang15/stype-trainer';
        },
    });
    MenuTitle = Menu.extend({
        ...
        scroll: 0,
        background: new ig.Image('media/background/stars.jpg'),
        stype: new ig.Image('media/title/stype.png'),
        ...
        draw: function() {
            ...
            this.stype.draw((ig.system.width / this.scale - this.stype.width) / 2, 150 - ig.ease.inOutQuad(this.transition, 0, 1000, 1));
...
ig.module('game.document-scanner').defines(function() {
    ...
    ig.DocumentScanner = ig.Class.extend({
        ...
        highlightClass: 'stype-current-text-fragment',
        init: function(rootElement) {
            ...
            this.$scanOverlay = ig.$('#stype-overlay');
            this.$scanProgress = ig.$('#stype-scan-progress');
            this.$scanOverlay.className = 'stype-scanning';
        ...
        scanComplete: function() {
            ...
            this.$scanOverlay.className = 'stype-scan-done';
            ...
            setTimeout((function() {
                ...
                this.$scanOverlay.className = 'stype-playing';
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.pause', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function() {
    ...
    stype = ig.Game.extend({
        ...
        reset: function() {
            ...
            this.wave = ig.copy(stype.WAVES[this.difficulty]);
        ...
        keypress: function(ev) {
            if (ev.target.tagName == 'INPUT' || ev.ctrlKey || ev.altKey || this.mode != stype.MODE.GAME || this.menu) {
                return true;
            }
            var c = ev.charCode;
            if (c < 64) {
        ...
        virtualKeydown: function(letter) {
            if (this.mode != stype.MODE.GAME || this.menu) {
                return true;
            }
            if (letter == 'ENTER') {
                this.player.spawnEMP();
        ...
        shoot: function(letter) {
            ...
            if (this.currentTarget) {
                ...
                if (hit) {
                    ...
                    if (stype.MULTIPLIER_TIERS[this.streak]) {
                        this.multiplier = stype.MULTIPLIER_TIERS[this.streak];
      ...
      // === NAYA BADLAV: GAME OVER HONE PAR RECORDING BAND KAREIN ===
            ...
            this.mode = stype.MODE.GAME_OVER;
            ig.music.fadeOut(1);
        },
        ...
        update: function() {
            ...
            if (this.mode === stype.MODE.GAME) {
                ...
                if (this.mode === stype.MODE.TITLE) {
                    this.setGame();
                } 
                else if ((this.mode === stype.MODE.GAME_OVER && this.menu && this.menu.timer.delta() > 1.5) || this.mode !== stype.MODE.GAME_OVER) {
                    this.setTitle();
                }
            }
            var scrollSpeed = 100;
            ...
            if (this.entities.length > 1 && this.mode == stype.MODE.GAME) {
        ...
        draw: function() {
            if (this.mode == stype.MODE.GAME || this.mode === stype.MODE.GAME_OVER) {
                this.drawGame();
            }
            if (this.menu) {
                this.menu.draw();
    ...
    var canvas = document.getElementById('stype-game-canvas');
    var width = 480;
    var height = 720;
    if (ig.ua.mobile) {
        ig.$('#stype-gsense').style.display = 'none';
        ig.$('#stype-byline').style.display = 'none';
        var resize = function() {
    ...
    if (window !== window.top || window.location.href.match(/\?gp=1/)) {
        var ad = ig.$('#stype-gsense');
        if (ad) {
            ad.className = 'stype-gsense-full';
        }
    }
    if (window.stypeDocumentMode) {
    ...
    ig.main('#stype-game-canvas', stype, 60, width, height, 1, ig.RiseLoader);
});